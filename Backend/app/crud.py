...
def create_item(
    ...
    cat = get_category(db, data.category_id, owner_id)
    if cat is None:
        return None
...
def update_item(
    db: Session, item: models.InventoryItem, data: schemas.ItemUpdate, owner_id: str
) -> Optional[models.InventoryItem]:
    cat = get_category(db, data.category_id, owner_id)
    if cat is None:
        return None
...
def get_dashboard_stats(db: Session, owner_id: str) -> schemas.DashboardStats:
    ...
    category_count = sum(len(inv.categories) for inv in inventories)
